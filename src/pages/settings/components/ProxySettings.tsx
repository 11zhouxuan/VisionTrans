  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-gray-700">{t('proxy.title')}</h3>
        <button onClick={handleToggle}
          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${enabled ? 'bg-blue-500' : 'bg-gray-300'}`}>
      ...
      {enabled && proxy && (
        <>
          <div>
            <label className="block text-xs text-gray-500 mb-1">{t('proxy.protocol')}</label>
            <select value={proxy.protocol} onChange={(e) => onProxyChange({ ...proxy, protocol: e.target.value as 'http' | 'socks5' })}
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">{t('proxy.address')}</label>
            <input type="text" value={proxy.url} onChange={(e) => onProxyChange({ ...proxy, url: e.target.value })}
